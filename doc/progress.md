...
4. **小程序同步与集成 (Mini-Program Integration)**:
   ...
   - **登录页极简重构**: 将背景改为纯黑色（#000000），移除了“登录权益”列表，并显著增大了 Logo、欢迎语和登录按钮的尺寸，提升了视觉冲击力。
   - **新增退出登录**: 在“会员”中心页面底部直接添加了“退出登录”按钮，方便用户切换账号或清理缓存。
   - **登录页自主配置**: 登录页新增了“更换头像”和“昵称输入”功能。用户可以在登录前自主选择头像和昵称，解决了微信隐私政策导致的默认匿名信息问题。
   - **修复预约报错**: 修复了小程序预约时出现的 400 校验错误。后端新增了 `seatNumber` 座位号存储，前端对齐了 `CreateReservationDto` 所需的必填字段（昵称、电话、桌台 ID、预约时间等）。
